...
describe('VideoPage Unit Tests', () => {
    ...
    it('handles delete video action', async () => {
        ...
        (YouTubeService.getVideos as any).mockResolvedValue({ videos: [] });
        (YouTubeService.getChannels as any).mockResolvedValue({ channels: [] });
        (YouTubeService.deleteVideo as any).mockResolvedValue(undefined);
        ...
        await waitFor(() => {
            expect(YouTubeService.deleteVideo).toHaveBeenCalledWith('vid1');
        });
    });

    it('handles resync video action', async () => {
        (YouTubeService.getVideo as any).mockResolvedValue(createVideo());
        (YouTubeService.getVideos as any).mockResolvedValue({ videos: [] });
        (YouTubeService.getChannels as any).mockResolvedValue({ channels: [] });
        (YouTubeService.resyncVideo as any).mockResolvedValue(undefined);
        ...
        await waitFor(() => {
            expect(YouTubeService.resyncVideo).toHaveBeenCalledWith('vid1');
